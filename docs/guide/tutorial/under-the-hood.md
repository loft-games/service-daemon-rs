...
*   **Lock Tracing (Macro Illusion)**: When you use `#[service]` or `#[trigger]`, the macro automatically redirects standard `RwLock` and `Mutex` to our tracked versions. This allows the system to monitor lock contention and automatically notify the `Watch` triggers when state changes.
...
[**<- Previous Step: Unit Testing & Simulation**](./unit-testing.md) | [**Next Step: Custom Trigger Implementation ->**](./tailor-made-triggers.md)
