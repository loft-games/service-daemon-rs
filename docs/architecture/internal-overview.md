...
`RegistryBuilder` asks the global catalog for a tag-filtered projection instead of scanning `SERVICE_REGISTRY` directly. The projection preserves registry order and original `ServiceEntryId` values, then `Registry` materializes the selected entries using the existing auto-start behavior.
...
| `ServiceDescription` | Public entry-scoped daemon-local definition view. It owns the static service metadata view and setup-time `instance_ids()` for entries materialized before instance handles are requested. |
| `ServiceHandle` | Public daemon-created handle to a selected service definition. It carries daemon identity plus a weak daemon-local control reference, exposes the selected static metadata, and can list runtime instances for that service without containing a `ServiceInstanceId`. |
| `ServiceInstanceHandle` | Public handle to a materialized runtime instance owned by one daemon. It contains instance identity, static service metadata, daemon identity, and a weak daemon-local control reference for instance status, runtime facts, trigger facts, and stop requests. |
