...
### Registry identity
Each selected entry receives a `ServiceEntryId` from its original `SERVICE_REGISTRY` index. Entries without a `#[input]` parameter materialize one singleton runtime instance when the registry is built. Entries with a single `#[input]` parameter stay selected but do not materialize an instance until a daemon-bound `ServiceHandle` creates one with typed input. Every materialized instance receives a fresh UUIDv7-backed `ServiceInstanceId`, and that runtime instance ID is used as the primary key in the Status Plane, Shelf, reload signals, runtime facts, diagnostics, and trigger routing. `ServiceEntryId` identifies the static registry entry; it cannot be converted into a `ServiceInstanceId`.

### Catalog projections and service handles
`RegistryBuilder` asks the global catalog for a tag-filtered projection instead of scanning `SERVICE_REGISTRY` directly. The projection preserves registry order and original `ServiceEntryId` values, then `Registry` materializes selected services without `#[input]` as startup instances while keeping selected template services available as service definitions.

Each `ServiceDaemon` stores its final projection in `DaemonResources`. Provider initialization can resolve `service_handle!(path::to::service)` against that daemon-local projection. The macro targets the wrapper function generated by `#[service]`; the runtime uses the wrapper pointer to find the static entry and returns a daemon-created `ServiceHandle`. The handle identifies the selected service definition inside that daemon, carries the daemon identity plus a weak daemon-local control reference, and does not contain a `ServiceInstanceId`. `ServiceHandle::create(input).await` creates a new daemon-local `ServiceInstanceRecord`, stores typed startup input for template services, registers runtime facts, and returns a `ServiceInstanceHandle` without starting the supervisor pipeline. `ServiceInstanceHandle::start().await` starts the registered instance. `ServiceHandle::start(input).await` is the create-and-start convenience path. Create requests require `run()` to have started; requests submitted while startup waves are still running wait until startup completes before registering the instance.

If the target service is linked but not selected by the daemon's registry projection, handle resolution fails with provider-init semantics. If the target wrapper is not linked into `SERVICE_REGISTRY`, resolution also fails. Calling the macro outside a daemon provider scope has no daemon projection and fails.

### Daemon-local instance registry
Each materialized registry entry stores its instances in a daemon-local instance registry keyed by `ServiceInstanceId`, with a secondary entry-to-instances index keyed by `ServiceEntryId`. Services without `#[input]` begin with one instance; template services begin with none and gain records through `ServiceHandle::create(input)` or the create-and-start convenience `ServiceHandle::start(input)`. Runner startup, shutdown, runtime facts, diagnostics, status updates, and instance control use the registry's instance records rather than treating a service description as a single running instance.

`ServiceDescription` is an entry-scoped daemon-local definition view. It keeps the selected `ServiceEntryId` and `&'static ServiceEntry`, exposes static metadata needed to materialize instances, and tracks the current daemon-local instance IDs for that service definition. The daemon-local instance registry stores internal records, not public handles. `DaemonInstanceHandle`, `ServiceHandle`, and `SimulationHandle` create bound `ServiceInstanceHandle` values from those records when callers list or query instances.

A `ServiceInstanceHandle` carries the runtime `ServiceInstanceId`, static `ServiceEntryId`, `&'static ServiceEntry`, daemon identity, and a weak daemon-local control reference. Callers can address one runtime instance, group it back to its static service definition, and perform instance-level reads, stop requests, graceful stop-and-wait, or removal without passing the daemon handle again. After the owning daemon is unregistered, stale instance handles keep their identity and static metadata but cannot operate on daemon state.
...
| `ServiceDescription` | Public entry-scoped daemon-local definition view. It owns the static service metadata view and exposes current daemon-local `instance_ids()` for that service definition. |
| `ServiceHandle` | Public daemon-created handle to a selected service definition. It carries daemon identity plus a weak daemon-local control reference, exposes the selected static metadata, can list runtime instances for that service, and can create or start new runtime instances without containing a `ServiceInstanceId`. |
...
| `SchedulingAdvisoryProfile` | Public advisory emission control only; it does not change lifecycle, body placement, declared scheduling, or HighPriority runtime control behavior. |
| HighPriority runtime control | Internal placement, pressure, cooldown, capacity, scale-out, and cooperative rollover loop for declared `HighPriority` generations; no daemon-level public policy handle is exposed. |
| HighPriority capacity plan | Internal startup topology decision derived from the final declared HighPriority entries; later scale-out is governed by internal HighPriority runtime control. |
