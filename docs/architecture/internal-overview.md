...
Each materialized registry entry stores its instances in a daemon-local instance registry keyed by `ServiceInstanceId`, with a secondary entry-to-instances index keyed by `ServiceEntryId`. Auto-start entries begin with one instance; non-auto-start entries begin with none and gain records through `ServiceHandle::create()` or the create-and-start convenience `ServiceHandle::start()`. Runner startup, shutdown, runtime facts, diagnostics, status updates, and instance control use the registry's instance records rather than treating a service description as a single running instance.
