...
Each `ServiceDaemon` stores its final projection in `DaemonResources`. Provider initialization can resolve `service_handle!(path::to::service)` against that daemon-local projection. The macro targets the wrapper function generated by `#[service]`; the runtime uses the wrapper pointer to find the static entry and returns a `ServiceHandle` containing `{ entry_id, entry }`. The handle identifies the service definition, not a running instance, does not contain a `ServiceInstanceId`, and is reusable with any daemon whose projection selects the same entry.
