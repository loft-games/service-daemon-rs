...
Provider-init failures currently retain the public `ProviderInitError::{Fatal, Timeout, Cancelled}` shape, while internal tracing can classify the generated wrapper path that observed the error:

| Source | Public boundary | Notes |
| :--- | :--- | :--- |
| User provider returns `ProviderError::Fatal` | `ProviderInitError::Fatal` | No retry; treated as daemon-wide provider-init failure. |
| User provider returns `ProviderError::Retryable` until timeout | `ProviderInitError::Timeout` | Retry/backoff remains bounded by `provider_init_timeout`. |
| Required `env` missing or parse failure | `ProviderInitError::Fatal` | Framework-owned configuration failure generated by the macro. |
| Dependency provider failure | Propagated `ProviderInitError` | The downstream provider remains the source of the terminal error; service identity is recorded by the supervisor, not duplicated in the provider error. |
| Initialization cancellation | `ProviderInitError::Cancelled` | Cancellation stays distinct from fatal failure. |
| Provider init panic | `ProviderInitError::Fatal` | `catch_init_panic` converts string and non-string panic payloads. |
| Eager dependency graph defense error | `ProviderInitError::Fatal` | Defensive startup path for impossible graph inconsistencies. |

The generated wrapper path (`snapshot_resolve`, `rwlock_resolve`, `mutex_resolve`, or `eager_init`) is diagnostic context, not a public control surface.
