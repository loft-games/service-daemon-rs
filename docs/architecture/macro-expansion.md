...
or `ServiceHandle::start(input)` with an owned `T`. The generated
`ServiceEntry` stores the optional input descriptor, including the input
parameter name and `T` type identity.
