...
The returned `ServiceHandle` contains the selected static entry's `ServiceEntryId` and `&'static ServiceEntry`; it does not contain a `ServiceInstanceId` and is not tied to one daemon instance. The current daemon projection decides whether the linked service is available to that daemon. Calling the macro outside a daemon scope fails because there is no daemon projection to resolve against.
