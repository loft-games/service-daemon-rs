...
`service_handle!(path::to::worker_service)` is a function-like macro for provider code that needs a daemon-local handle to a service definition. The macro rewrites the service function path to the wrapper symbol generated by `#[service]` and calls the runtime resolver. It returns `Result<ServiceHandle, ProviderError>`.

The returned `ServiceHandle` contains the selected static entry's `ServiceEntryId` and `&'static ServiceEntry`; it does not contain a `ServiceInstanceId`. The current daemon projection decides whether the linked service is available to that daemon. Calling the macro outside a daemon provider scope fails because there is no daemon-local projection to resolve against.
