...
2. **source_service_instance_id** (`ServiceInstanceId`): the service instance that originally published the event.
3. **Trigger invocation identity**:
...
If handler B publishes event Y while handling event X, event Y gets a new UUID v7 `message_id` and keeps the original `source_service_instance_id`. This lets diagnostics link a cascade of events back to the service instance that started it, even when the work crosses services or queues.
