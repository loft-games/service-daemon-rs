...
- **Service Template Selection**: Removed `#[service(auto_start = ...)]` as a public service attribute. Selected services without `#[input]` still create one startup instance; selected services with `#[input]` start with no instances and must be created with typed input.
