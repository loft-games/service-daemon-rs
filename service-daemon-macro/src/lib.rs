...
/// Resolves a daemon-local handle for a registered service.
///
/// The macro accepts a service function path and rewrites it to the wrapper
/// symbol generated by `#[service]`.
///
/// # Example
/// ```rust,ignore
/// #[provider]
/// pub fn worker_handle() -> Result<ServiceHandle, ProviderError> {
///     service_daemon::service_handle!(worker_service)
/// }
/// ```
#[proc_macro]
pub fn service_handle(input: TokenStream) -> TokenStream {
    let mut path = syn::parse_macro_input!(input as syn::Path);

    let Some(last_segment) = path.segments.last_mut() else {
        return TokenStream::from(
            syn::Error::new(
                proc_macro2::Span::call_site(),
                "expected a service function path",
            )
            .to_compile_error(),
        );
    };
    ...
    if !matches!(last_segment.arguments, syn::PathArguments::None) {
        return TokenStream::from(
            syn::Error::new_spanned(
                ...
                "service_handle! expects a plain service function path",
            )
            .to_compile_error(),
        );
    }

    let wrapper_ident = format_ident!("{}_wrapper", last_segment.ident);
    last_segment.ident = wrapper_ident;

    TokenStream::from(quote! {
        service_daemon::__private::__resolve_service_handle(#path)
    })
}
