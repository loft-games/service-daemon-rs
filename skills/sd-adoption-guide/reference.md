...
4. **HighPriority only when the work is cooperative.** For latency-sensitive async
   workers, declare `scheduling = HighPriority` and let the daemon manage
   placement internally. The framework can add shards and request cooperative
   generation rollover, but it cannot make blocking or CPU-heavy code safe.
